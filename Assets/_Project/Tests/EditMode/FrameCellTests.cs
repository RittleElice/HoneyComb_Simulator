...
namespace HoneyComb.Tests {
 public sealed class FrameCellTests {
  [Test] public void FacesAndCoordinatesAreIndependent(){
   ...
   Assert.That(f.TrySetCell(FrameSide.Front,89,39,new CellState(100,CellContentType.Honey)),Is.True);
   ...
   var copy=f.GetCell(FrameSide.Front,89,39).Value;copy=new CellState(100,CellContentType.Pollen);
   Assert.That(f.GetCell(FrameSide.Front,89,39).Value.Content,Is.EqualTo(CellContentType.Honey));
  }
  [Test] public void StateSurvivesFrameMovementAndReinsertion(){
   ...
   f.TrySetCell(FrameSide.Back,3,2,new CellState(100,CellContentType.Pollen));
  ...
  [Test] public void InvalidCoordinatesDoNotWrite(){
   var f=new Frame("f");var honey=new CellState(100,CellContentType.Honey);
